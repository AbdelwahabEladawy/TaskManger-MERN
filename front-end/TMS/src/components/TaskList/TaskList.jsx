...
function TaskList({ tableData }) {
    console.log(tableData);
    
    const taskStatusBadgeColor = (status) => {
        switch (status) {
    ...
    const taskPriorityBadgeColor = (priority) => {
        switch (priority) {
    ...
    return (
        <div className="overflow-x-auto p-0 rounded-lg mt-3 ">
            <table className='min-w-full '>
                ...
                <tbody>
                    {tableData.map((task) => {
                      return  <tr key={task._id} className=''>
                            <td className='my-3 mx-4  text-gray-700 text-[13px] line-clamp-1 overflow-hidden'>{task.title}</td>
                            <td className='px-4 py-4'>
                                <span className={`px-2 text-xs rounded inline-block ${taskStatusBadgeColor(task.status)}`}>{task.status}</span>
                            </td>
                            <td className='px-4 py-4'>
                                <span className={`px-2 text-xs rounded inline-block ${taskPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                            </td>
                            <td className='px-4 py-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell'>{task.createdAt ? moment(task.createdAt).format("Do MM YYYY") : "N/A"}</td>
                        </tr>
                    })}
                </tbody>
            </table>
